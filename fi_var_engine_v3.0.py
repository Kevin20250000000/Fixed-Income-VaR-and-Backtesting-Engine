...
class HullWhiteModel:
    ...
    def simulate_paths(self, n_paths, n_steps, dt, seed=42):
        ...
        for s in range(n_steps):
            t     = max(s * dt, 1e-6)
            theta = (self.a * self._fwd(t)
                     + float(self._cs(np.clip(t, self._t_min, self._t_max), 2))
                     + self.sigma**2 / (2.0 * self.a) * (1.0 - np.exp(-2.0 * self.a * t)))
            r[:, s+1] = r[:, s] + (theta - self.a * r[:, s]) * dt + self.sigma * np.sqrt(dt) * Z[:, s]
        return r
...
def _reprice_coupon_bond_mc(notional, coupon, maturity_T, credit_spread, model, r_h, t_h, freq=2):
    ...
    if len(t_cf) == 0: return np.zeros(len(r_h))
    p = np.zeros(len(r_h))
...
def _reprice_mbs_psa_mc(notional, coupon, wac, cpr_base, maturity_years,
    ...
    for i in range(n_months):
        t           = t_h + (i + 1) / 12.0
        cpr_m       = psa_ramp[i] * cpr_eff                              # (n_paths,)
...
def compute_portfolio_pnl(positions_df, model, sim_cfg):
    ...
    r_paths   = model.simulate_paths(n_paths, n_steps, dt, seed=sim_cfg["seed"])
    r_horizon = r_paths[:, horizon]
    ...
    return portfolio_pnl, market_value, r_paths
