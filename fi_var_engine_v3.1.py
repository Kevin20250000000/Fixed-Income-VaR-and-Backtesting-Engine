"""
Fixed Income VaR & Backtesting Engine  v3.0
============================================
Run:  python3 fi_var_engine.py
...
Pipeline:
  ...
  3. Rate Models            — Vasicek / Hull-White + Antithetic Variates MC
  ...
  6. Model Validation       — Kupiec POF, Christoffersen CC, Basel Traffic Light,
                              HS benchmark, parameter sensitivity
  7. SR 11-7 Documentation  — auto-generated Markdown report
...
YIELD_CURVE = {                        # tenor (yrs) → continuously-compounded zero rate
...
class VasicekModel:
    ...
    def simulate_paths(self, n_paths, n_steps, dt, seed=42):
        ...
        std_step = np.sqrt(self.sigma**2 / (2.0 * self.kappa) * (1.0 - np.exp(-2.0 * self.kappa * dt)))
...
class HullWhiteModel:
    ...
    def __init__(self, p, yield_curve):
        ...
        self._cs = CubicSpline(t_aug, ld_aug, bc_type="not-a-knot")
        self._t_min, self._t_max = t_aug[0], t_aug[-1]

    def _ld(self, t):  return float(self._cs(np.clip(t, self._t_min, self._t_max)))
    def _fwd(self, t): return float(-self._cs(np.clip(t, self._t_min, self._t_max), 1))
    ...
    def simulate_paths(self, n_paths, n_steps, dt, seed=42):
        ...
        t_arr     = np.maximum(np.arange(n_steps, dtype=float) * dt, 1e-6)
        t_clipped = np.clip(t_arr, self._t_min, self._t_max)
        fwd_arr   = -self._cs(t_clipped, 1)                           # f^M(0,t)  (n_steps,)
        d2ld_arr  =  self._cs(t_clipped, 2)                           # d²ln P/dt²
        theta_arr = (self.a * fwd_arr + d2ld_arr
                     + self.sigma**2 / (2.0 * self.a)
                     * (1.0 - np.exp(-2.0 * self.a * t_arr)))         # (n_steps,)

        sig_sdt = self.sigma * np.sqrt(dt)
    ...
    def discount_factor(self, r_h, t_h, T):
        ...
        return np.exp(self._ld(T) - self._ld(t_c) - B * (r_h - self._fwd(t_c)) + adj)
    ...
    def discount_factor_batch(self, r_h, t_h, times):
        ...
        ln_ratio   = self._cs(Tv_clipped) - self._ld(t_h_c)           # (n_valid,)

        # ln P(t_h, T | r) = ln_ratio[i] + adj[i] + B[i]*f_t − B[i]*r_h[j]
        ln_P = (ln_ratio + adj + B * f_t)[:, None] - B[:, None] * r_h[None, :]
    ...
    def simulate_horizon(self, n_paths, horizon_days, dt, seed=42):
        ...
        fwd0    = self._fwd(t0)
        d2ld0   = float(self._cs(np.clip(t0, self._t_min, self._t_max), 2))
...
def _price_coupon_bond(notional, coupon, maturity_T, credit_spread, zcb_fn, freq=2):
    ...
    return float(sum(cf * zcb_fn(t) * np.exp(-credit_spread * t) for t, cf in zip(t_cf, cfs)))
...
def _reprice_coupon_bond_mc(notional, coupon, maturity_T, credit_spread, model, r_h, t_h, freq=2):
    ...
    for t, cf in zip(t_cf, cfs):
        p += cf * model.discount_factor(r_h, t_h, t) * np.exp(-credit_spread * (t - t_h))
    return p


def _reprice_structured_mc(notional, eff_dur, product_type, r_h, r0):
...
def _reprice_mbs_psa_mc(notional, coupon, wac, cpr_base, maturity_years,
    ...
    cpr_base      : baseline annual CPR at full PSA ramp (e.g. 0.10 = 100% PSA × 6%)
    ...
    beta_refi = 8.0
    cpr_eff   = np.clip(cpr_base * (1.0 + beta_refi * (model.r0 - r_horizon)),
                        cpr_base * 0.10, cpr_base * 4.0)        # (n_paths,)

    psa_ramp = np.minimum(np.arange(1, n_months + 1) / 30.0, 1.0)  # (n_months,)
    ...
    for i in range(n_months):
        ...
        cf_buf.append(interest + total_prin)
        t_buf.append(t)
        bal = np.maximum(bal - total_prin, 0.0)
        if np.all(bal < 1.0):
            break
...
def _reprice_mbs_psa_static(notional, coupon, wac, cpr_base, maturity_years,
    ...
    beta_refi = 8.0
    cpr_eff   = float(np.clip(cpr_base * (1.0 + beta_refi * (r0 - r_proxy)),
    ...
    for i in range(n_months):
        ...
        y          = float(np.atleast_1d(yc_fn(t))[0])
        df         = np.exp(-y * t - credit_spread * t)
        price     += cf * df
...
def compute_portfolio_pnl(positions_df, model, sim_cfg):
    ...
    r_horizon   = model.simulate_horizon(n_paths, horizon, dt, seed=sim_cfg["seed"])
    ...
    def mkt_zcb(t):
        return np.exp(-float(np.atleast_1d(ns_fn(t))[0]) * t)
    ...
    for _, pos in positions_df.iterrows():
        ptype, notl = pos["product_type"], pos["notional"]
...
def calculate_var_cvar(pnl_array, confidence=None):
    ...
    var  = float(-np.percentile(pnl_array, (1.0 - confidence) * 100.0))
    tail = pnl_array[pnl_array <= -var]
    return var, (float(-tail.mean()) if len(tail) > 0 else var)
...
def run_stress_tests(positions_df):
    ...
    mkt_zcb  = lambda t: np.exp(-float(np.atleast_1d(ns_base(t))[0]) * t)
        ...
        for _, pos in positions_df.iterrows():
            ...
            if ptype == "mbs":
                ...
                total += _price_coupon_bond(pos["notional"], pos["coupon"],
                                            pos["maturity_years"], pos["credit_spread"], zcb)
        return total
    ...
    for scen_name, shocks in STRESS_SCENARIOS.items():
        ...
        s_ylds = np.array(yc_yields) + np.array([shocks.get(t, 0.0) for t in yc_tenors])
        ns_shocked = _fit_nelson_siegel(yc_tenors, s_ylds.tolist())
        # Representative rate proxy for PSA sensitivity (notional-weighted avg shock)
        dr_mean = float(np.mean([shocks.get(t, 0.0) for t in yc_tenors]))
        r_proxy = r0 + dr_mean

        results[scen_name] = price_portfolio(ns_shocked, r_proxy) - baseline
...
def run_backtesting(historical_pnl, var_estimate, alpha=None):
    ...
    if 0 < p_hat < 1:
        ...
        ku_pval = float(1.0 - stats.chi2.cdf(lr_uc, df=1))
    else:
    ...
    eps   = 1e-10
    pi_01 = n01 / max(n00+n01, 1);  pi_11 = n11 / max(n10+n11, 1)
    pi    = (n01+n11) / max(T-1, 1)
    sl    = lambda x: np.log(max(x, eps))
    ...
    n_250 = int(round(N*250/T)) if T != 250 else N
...
def sensitivity_analysis(positions_df, model_name, base_var, base_cvar):
    ...
    for label, params in perturb:
        if label == "Baseline":
            ...
            continue
        pnl, _, _ = compute_portfolio_pnl(positions_df, build(params), sim_fast)
...
> Engine v3.0 as of {today}. The following enhancements have been implemented relative to prior
...
*Fixed Income VaR Engine v3.0 | {today} | SR 11-7 | OCC 2011-12 | Basel III IMA*
...
def main():
    print(f"\n{DSEP}")
    print("  Fixed Income VaR & Backtesting Engine  v3.0")
    print("  SR 11-7 | Basel III IMA | Hull-White / Vasicek")
    print("  Nelson-Siegel | PSA Prepayment | Antithetic Variates")
    print(DSEP)
    ...
    _hdr("6b / 8  α×σ VaR Sensitivity Heatmap")
    ...
    # Documentation
    _hdr("6c / 8  SR 11-7 Model Risk Documentation")
